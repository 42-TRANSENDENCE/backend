...
export class GameService {
  makeNewRoom(roomId: string, p1: string, p2: string): void {
  ...
  handleKeyPressed(roomId: string, socketId: string, keyCode: string): void {
    const game = this.__game_list.get(roomId);
    if (socketId === game.players.p1) {
      if (keyCode === 'ArrowUp') game.data.up_pressed.p1 = true;
      else if (keyCode === 'ArrowDown') game.data.down_pressed.p1 = true;
    } else if (socketId === game.players.p2) {
      if (keyCode === 'ArrowUp') game.data.up_pressed.p2 = true;
      else if (keyCode === 'ArrowDown') game.data.down_pressed.p2 = true;
    }
  }

  handleKeyReleased(roomId: string, socketId: string, keyCode: string): void {
    const game = this.__game_list.get(roomId);
    if (socketId === game.players.p1) {
      if (keyCode === 'ArrowUp') game.data.up_pressed.p1 = false;
      else if (keyCode === 'ArrowDown') game.data.down_pressed.p1 = false;
    } else if (socketId === game.players.p2) {
      if (keyCode === 'ArrowUp') game.data.up_pressed.p2 = false;
      else if (keyCode === 'ArrowDown') game.data.down_pressed.p2 = false;
    }
  }

  handlePlayerReady(nsp: Namespace, roomId: string, socketId: string): void {
    const game = this.__game_list.get(roomId);
    if (game) {
      if (socketId === game.players.p1) game.isReady.p1 = true;
      else if (socketId === game.players.p2) game.isReady.p2 = true;
  ...
  quitGame(nsp: Namespace, socketId: string): void {
    const game: GameType | undefined = this.__player_list.get(socketId);
    // 1. 소켓 아이디로 플레이어의 게임ID를 찾는다.
    if (game === undefined) return;

    // 2. 해당 게임에서 나가는 플레이어의 점수를 -1로 바꾼다.
    if (game.players.p1 === socketId) game.data.score.p1 = -1;
    else if (game.players.p2 === socketId) game.data.score.p2 = -1;
  ...
  /* method */
  private __start_game(nsp: Namespace, game: GameType): void {
    console.log('loop 시작');
  ...
  private __stop_game(game: GameType): void {
  ...
  private __single_game_frame(nsp: Namespace, game: GameType): void {
  ...
  private __keyboard_check(game: GameType): void {
    const p1_dir: number =
      Number(game.data.down_pressed.p1) - Number(game.data.up_pressed.p1);
    const p2_dir: number =
      Number(game.data.down_pressed.p2) - Number(game.data.up_pressed.p2);
  ...
  private __collid_check(game: GameType): void {
  ...
  private __score_check(nsp: Namespace, game: GameType): void {
  ...
  private __wall_collision(positions: GameDataType): void {
    const pos = positions.ball_pos;
    const vel = positions.ball_vel;
  ...
  private __paddle_collision(player: string, game: GameType): void {
    let center;
    const vel = game.data.ball_vel;
